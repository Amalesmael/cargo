...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Validate salary
    $input_salary = trim($_POST["jope"]);
    if(empty($input_salary)){
        $salary_err = "Please enter the jop.";
    }  else{
        $salary = $input_salary;
    }
    ...
    if(empty($name_err) && empty($jope_err) && empty($email_err)){
        ...
        $sql = "INSERT INTO employees (emp_name, emp_email, emp_jope,emp_password,bra_id) VALUES (?, ?, ?,?,?)";

        if($stmt = $connection->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sssss", $_POST["name"], $_POST["email"], $_POST["jope"],$_POST['password'],$bra_id);
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <div class="form-group">
                        <label>jope</label>
                        <input type="text" name="jope" class="form-control <?php echo (!empty($jope_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $jope; ?>">
                    ...
                    <a href="employees.php" class="btn btn-danger ml-2">Cancel</a>
                </form>
            </div>
        </div>
    </div>
